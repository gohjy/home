...
let query = new URL(location.toString()).searchParams;
let jwt = query.get("jwt");
let userJWT = parseJwt(jwt);

let userID = userJWT.em.slice(0, 8);
...
.then(function(res) {
    ...
    if (user === null) {
		if (/^h\d{7}$/.test(userID)) {
			...
			localStorage.setItem("userData-loginv2", user);
			let goto = localStorage.getItem("login_goto");
			localStorage.removeItem("login_goto");
        ...
        localStorage.setItem("userData-loginv2", userJSON);
