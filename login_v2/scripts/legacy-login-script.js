...
let classes = {
	GEP: {
		"MS-EQUIV-102": {
			name: "MS-EQUIV-102-ERROR-ONLY",
			mixedFC: ["MFC-EQUIV-102", "MFC-EQUIV-102-ALT"],
			mainstreamEquiv: "M25102",
			type: "ms"
		},
		"M25102": {
			name: "M25102",
			mixedFC: ["MFC-EQUIV-102", "MFC-EQUIV-102-ALT"],
			mainstreamEquiv: "MS-EQUIV-102"
		}
	},
	mixedFC: {
		"MFC-EQUIV-102": {
			name: "MFC-EQUIV-102-ERROR-ONLY",
			mainstreamEquiv: "MS-EQUIV-102",
			GEPequiv: "M25102"
		},
		"MFC-EQUIV-102-ALT": {
			name: "MFC-EQUIV-102-ALT-ERROR-ONLY",
			mainstreamEquiv: "MS-EQUIV-102",
			GEPequiv: "M25102"
		}
	},
...
let users = [
	{
		...
		GEPclass: classes.GEP["M25102"],
		mixedFC: classes.mixedFC["MFC-EQUIV-102"],
	}
]


function usernameSearch(username, userlist) {
...
async function loginsys(username, password) {
	let statusref = document.querySelector("#status")
	let userlist = await fetch(new Request("/home/login_v2/scripts/users.json"));
	let userjson = await userlist.json();

	let user = usernameSearch(username, userjson)
	if (user == -1) {
